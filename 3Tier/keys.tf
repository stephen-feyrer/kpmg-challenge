...
locals {
  ...
  key_file   = pathexpand("~/.ssh/3Teir_key.pem")
}

locals {
  bash           = "chmod 400 ${local.key_file}"
  bash_ssh       = "eval `ssh-agent` ; ssh-add -k ${local.key_file}"
  powershell     = "icacls ${local.key_file} /inheritancelevel:r /grant:r test-user:R"
  powershell_ssh = "ssh-agent ; ssh-add -k ~/.ssh/3Teir_key.pem"
}
